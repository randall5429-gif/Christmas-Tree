...
function drawTopStar(x, y, r, glow) {
  ...
  for (let i = 0; i < 5; i++) {
    ctx.lineTo(
      Math.cos((i * 2 * Math.PI) / 5) * r,
      Math.sin((i * 2 * Math.PI) / 5) * r
    );
    ctx.lineTo(
      Math.cos(((i * 2 + 1) * Math.PI) / 5) * (r / 2),
...
/* ---------- Garland (RGB + Wave + Music) ---------- */
function drawGarland() {
  const time = Date.now();

  for (let j = 0; j < 3; j++) {
    const hue = (time * 0.05 + j * 120) % 360;
    const brightness = 50 + musicLevel * 0.5;
    ...
    for (let i = 0; i <= 500; i++) {
      const t = i / 500;
      const angle =
        t * GARLAND_TURNS * Math.PI * 2 -
        rotation -
        time * GARLAND_ANIMATION.speed * 0.001;

      const wave =
        Math.sin(time * GARLAND_ANIMATION.waveSpeed * 0.001 + t * 12) *
        (GARLAND_ANIMATION.amplitude + musicLevel * 0.1);
...
function animate() {
  ...
  rotation -= 0.003; // inverted rotation
